...
function Home() {
  ...
  try {
    ...
    if (status !== "accepted") throw new Error("Job not accepted");

    console.log(`Job ${job_id} started. Polling...`);

    // 2. Poll Status
    let attempts = 0;
    const maxAttempts = 100; // 200 seconds approx

    const poll = async () => {
      if (attempts > maxAttempts) throw new Error("Analysis timed out (Polling)");
      attempts++;
      ...
      if (jobData.status === "completed") {
        setData(parsAnalysis(jobData.data));
        return;
  ...
  // Note: If success, loading stays true until setData triggers re-render (or manage loading inside poll)
  // Actually, setData calls re-render, but loading state might persist if not cleared.
  // Let's modify:
  // We need to clear loading state when data is set.
  // The poll function is recursive/async opacity. 
  // Ideally we use a loop with await sleep.
};
